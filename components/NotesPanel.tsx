...
export function NotesPanel({ notes, onCreate, onView, onEdit, onRestore, onCopy, onDelete }: Props) {
  return (
    <div className="panel-body">
      ...
      {!notes.length && <div className="empty-state"><span>◇</span><h3>残しておきたい計算を保存</h3><p>計算結果のメモ、またはMarkdownで自由に書けるプレーン計算メモを作成できます。</p><button className="button-primary empty-create-button" onClick={onCreate}>最初のメモを作成</button></div>}
      <div className="note-list">
        {notes.map((note) => <article className="note-card" key={note.id}>
          ...
          <span className={`memo-kind ${note.type === "plain-calculation" ? "plain" : ""}`}>{note.type === "plain-calculation" ? "プレーン計算メモ" : "計算メモ"}</span>
